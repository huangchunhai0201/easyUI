$(document).ready(function(){
    ...
    centerContentLayer.layout({fit:true})
        .layout("add",{
            region:"west",
            split:true,
            content:"<div class='treeButtons' style='padding: 5px;border-bottom:1px solid #ccc;'></div><div id='roleTreeLayer' style='width:100%;'></div>",
            width:260})
        .layout("add",{
            region:"center",
    ...
    var roleTreeLayer = centerContentLayer.layout("panel","west").find("#roleTreeLayer");  //加载部门树的容器
    var tabsLayer = centerContentLayer.layout("panel","center").find("#tabsLayer");  //存放部门tab的容器

    //加载树操作按钮
    treeButoons.append(
        '<a id="createRoleBtn" href="#" class="easyui-linkbutton" data-options="iconCls:\'icon-add\'">新增角色</a>'+
            '&nbsp<a id="removeRoleBtn" href="#" class="easyui-linkbutton" data-options="iconCls:\'icon-remove\'">删除角色</a>'
    );
    ...
    roleTreeLayer.tree({
        ...
        onLoadSuccess:depart.roleTreeLoadCallback,
