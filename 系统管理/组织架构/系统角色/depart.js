...
(function(){
    depart = {
        roleTreeLoadCallback: function(){ //部门树加载成功后的回掉函数
            var roleTreeLayer = $("#roleTreeLayer");
            var node = roleTreeLayer.tree('find',100);
            roleTreeLayer.tree('select',node.target);

            // ----------此处放置左侧部门树的事件方法-----------------------
            $("#createRoleBtn").bind("click",function(){
                $("#tabsLayer").tabs("select",1);
                $("#form").form("clear");
            });
            $("#removeRoleBtn").bind("click",function(){
                var node = roleTreeLayer.tree('getSelected');
                roleTreeLayer.tree('remove',node.target);
            });
        },
        ...
        createMember: function(){    //往部门添加成员
            if($("#dlg_selectPeople").length>0){
                $("#dlg_selectPeople").dialog('open');
            }else{
                var $modal = $('<div id="dlg_selectPeople"></div>');
                $("body").append($modal);
                $modal.dialog({
                    width:400,
                    height:500,
        ...
        modifyPeopleInfo:function(rowsData){
            ...
            $("#modifyPeopleInfo").bind("click",function(){
                if($("#dlg_modifyPeopleInfo").length>0){
                    $("#dlg_modifyPeopleInfo").dialog('open');
                }else{
